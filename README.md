...
1. **Multiple Content Collections**: The site is organized into different content types (long_form, posts, muses, zeitweilig, authors), each managed as an Astro content collection. This allows for type-safe content management and easy querying.
...
Content is managed through Markdown files located in the `src/content/` directory. Each content type (e.g., long_form, posts) has its own subdirectory.
...
2. **Collection Routes**:
   For each collection (long_form, posts, muses, zeitweilig):
